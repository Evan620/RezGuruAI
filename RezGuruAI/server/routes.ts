...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  apiRouter.post("/workflows/:id/run", async (req, res) => {
    ...
    try {
      ...
      let results: any = { processed: 0, actions: [] };
      
      // Based on workflow.actions, perform different operations
      // Ensure workflow.actions is properly typed
        ...
        switch (action.type) {
          ...
          case "document":
            ...
            if (action.config?.template) {
              ...
              const leadId = userLeads[0].id; // Use first lead as example
          ...
          case "score":
            ...
            let scoredCount = 0;
            for (const lead of userLeads.slice(0, 3)) { // Process up to 3 leads as example
              try {
                const updatedLead = await storage.updateLead(lead.id, {
                  motivationScore: Math.floor(Math.random() * 30) + 70 // Simulate scoring
                });
                scoredCount++;
              } catch (e) {
          ...
          case "sms":
          case "call":
            actionResult.details = `Would send ${action.type} notification to filtered leads (simulated)`;
